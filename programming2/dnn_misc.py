...
class linear_layer:
    ...
    def backward(self, X, grad):
        ...
        self.gradient['W'] = np.dot(X.T, grad) #(input_D-by-output_D numpy array, the gradient of the mini-batch loss w.r.t. self.params['W'])  #
        self.gradient['b'] = np.sum(grad, axis=0, keepdims=True) #(1-by-output_D numpy array, the gradient of the mini-batch loss w.r.t. self.params['b'])        #
        backward_output = np.dot(grad, self.params['W'].T) #(N-by-input_D numpy array, the gradient of the mini-batch loss w.r.t. X)                           #
...
class dropout:
    ...
    def forward(self, X, is_train):
        ...
        if is_train:
            self.mask = (np.random.random(X.shape) >= self.r) / (1.0 - self.r)
            forward_output = X * self.mask
        else:
            forward_output = X
        return forward_output
